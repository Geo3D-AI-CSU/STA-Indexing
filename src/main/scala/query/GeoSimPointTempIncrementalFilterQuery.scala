...
import index.{Z3DEncoder, TimeBucket, GeoSimCoordMapper, Bounds, GeoSimTempBucket}
...
class GeoSimPointTempIncrementalFilterQuery(
  zkQuorum: String,
  ...
  def queryRawLinesByTemp(
    ...
    println("\n" + "=" * 80)
    println("[GeoSim-Point-Temp-Incremental-Query] Execution plan:")
  ...
  private def queryTempBucketIndex(bMin: Int, bMax: Int): Set[String] = {
    val table = connection.getTable(TableName.valueOf(HBaseTableManager.geoSimPointTempBucketIdxTableName(dataset)))
  ...
  private def fetchAndRefine(rowKeys: Set[String], tMin: Double, tMax: Double,
                             startMs: Long, endMs: Long,
                             xMin: Double, yMin: Double, zMin: Double,
  ...
  private def printExecutionPlan(step0Time: Long, step1Time: Long, step2Time: Long, step3Time: Long, step4Time: Long,
                                  ...
                                  totalTime: Long): Unit = {
    println("\n" + "=" * 80)
    println(s"[GeoSim-Point-Temp-Incremental-Query-Summary]")
